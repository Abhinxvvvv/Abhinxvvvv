<div align="center">

# `[ >_ ABHINAV_RATHORE.exe ]`

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Share+Tech+Mono&weight=500&size=18&duration=2000&pause=1000&color=00FFFF&center=true&vCenter=true&width=600&lines=>_SYSTEM_READY...;>_ROLE:+FULL_STACK_DEVELOPER;>_CLASS:+CYBER_SECURITY;>_PRESS_START_TO_CONTINUE" alt="Typing SVG" /></a>

<br />

<a href="https://linkedin.com/in/abhinav-rathore"><img src="https://img.shields.io/badge/B.Tech_Cyber_Security-000000?style=flat-square&logo=academia&logoColor=00FFFF&borderColor=00FFFF" alt="Academic" /></a>
<a href="#"><img src="https://img.shields.io/badge/Indore,_India-000000?style=flat-square&logo=googlemaps&logoColor=00FFFF&borderColor=00FFFF" alt="Location" /></a>
<br />
<a href="https://linkedin.com/in/abhinav-rathore"><img src="https://img.shields.io/badge/LinkedIn-000000?style=flat-square&logo=linkedin&logoColor=00FFFF&borderColor=00FFFF" alt="LinkedIn" /></a>
<a href="mailto:abhinavrathore9399@gmail.com"><img src="https://img.shields.io/badge/Email-000000?style=flat-square&logo=gmail&logoColor=00FFFF&borderColor=00FFFF" alt="Email" /></a>
<a href="https://github.com/abhinavvvv"><img src="https://img.shields.io/badge/GitHub-000000?style=flat-square&logo=github&logoColor=00FFFF&borderColor=00FFFF" alt="GitHub" /></a>

<br />
<br />

<img src="https://komarev.com/ghpvc/?username=abhinavvvv&label=VIEWS&color=00FFFF&style=flat-square&label_color=000000" alt="Profile Views" />
<img src="https://img.shields.io/github/followers/abhinavvvv?label=FOLLOWERS&style=flat-square&color=00FFFF&labelColor=000000" alt="Followers" />

</div>

---

## 🎮 `PLAYER_PROFILE`

> **CLASS:** Full Stack Developer | **SPECIALIZATION:** Cyber Security  
> **CURRENT_GUILD:** Acropolis Institute of Technology and Research (B.Tech)
> **OBJECTIVE:** Architecting scalable web infrastructures and resilient distributed systems. Accelerating platform performance while securing web traffic.

---

## 🎒 `INVENTORY // TECH_STACK`

### `>_ WEAPONS_OF_CHOICE (LANGUAGES)`
<a href="#"><img src="https://skillicons.dev/icons?i=java,c,cpp,py,js,cs&theme=dark" alt="Languages" /></a>

### `>_ ARMOR_&_GEAR (FRAMEWORKS & DB)`
<a href="#"><img src="https://skillicons.dev/icons?i=react,nodejs,express,dotnet,mongodb,mysql,sqlite&theme=dark" alt="Web Technologies" /></a>

### `>_ ARTIFACTS (TOOLS)`
<a href="#"><img src="https://skillicons.dev/icons?i=git,github,figma,unity,netlify,vercel&theme=dark" alt="Tools" /></a>

---

## 📜 `QUEST_LOG // EXPERIENCE`

```json
{
  "company": "Extrnxt Labs Pvt Ltd",
  "role": "Full Stack Developer",
  "duration": "Mar 2026 - Jul 2026",
  "achievements": [
    "Developed end-to-end web applications with robust back-end services.",
    "Engineered Python simulations for real-world data flows.",
    "Integrated RESTful APIs into scalable, production-ready systems."
  ]
}
